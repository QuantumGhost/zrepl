...
type Execer interface {
	...
	// RunExpectSuccess(ctx context.Context, cmd string, args ...string) (stdout string, err error)
}
...
func Run(ctx context.Context, rk RunKind, rootds string, stmtsStr string) error {
	...
	if data[begin] == '"' {
		...
		for end < len(data) {
			...
			if data[end-1] != '\\' {
				...
				withoutBaskslash := bytes.ReplaceAll(withBackslash, []byte("\\\""), []byte("\""))
